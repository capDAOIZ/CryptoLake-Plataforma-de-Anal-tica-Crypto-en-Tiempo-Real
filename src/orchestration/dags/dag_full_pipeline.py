...
if [ -n "${SPARK_CONTAINER:-}" ]; then
  ...
  PROJECT_NAME="$(docker inspect "$SELF_CONTAINER" | python -c 'import json,sys; print(json.load(sys.stdin)[0]["Config"]["Labels"].get("com.docker.compose.project",""))' 2>/dev/null || true)"
  ...
  if [ -z "$TARGET_CONTAINER" ]; then
    echo "ERROR: no running container found for service '${SPARK_SERVICE}' in project '${PROJECT_NAME}'."
